...
export default function AdminDonationsPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      {donations.length === 0 ? (
        <p className="text-text-secondary italic">Aucun don en attente.</p>
      ) : (
        <div className="space-y-4">
          {donations.map((d) => (
            <div
              key={d.id}
              className="flex items-center justify-between p-4 rounded-2xl bg-white dark:bg-white/5 border border-border dark:border-white/10"
            >
              <div>
