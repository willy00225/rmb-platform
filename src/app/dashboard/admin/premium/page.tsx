  ...
  return (
    <div className="space-y-8 animate-fadeInUp">
      ...
      <div className="rounded-2xl bg-white border border-border p-6">
        <h2 className="text-xl font-semibold text-text mb-4">Tarifs</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {configs.map((config: any) => (
            <div key={config.id} className="flex justify-between items-center p-4 rounded-xl bg-gray-50 border border-border">
              <div>
                <p className="font-medium text-text">{config.label}</p>
                <p className="text-sm text-text-secondary">{config.amount} FCFA</p>
              </div>
      ...
      <div className="rounded-2xl bg-white border border-border p-6">
        <h2 className="text-xl font-semibold text-text mb-4">Fonctionnalités</h2>
        <div className="space-y-3">
          {features.map((feature: any) => (
            <div key={feature.id} className="flex justify-between items-center p-4 rounded-xl bg-gray-50 border border-border">
              <div>
      ...
      {/* Abonnés */}
      <div className="rounded-2xl bg-white border border-border p-6">
        <h2 className="text-xl font-semibold text-text mb-4">Abonnés ({subscribers.length})</h2>
        {subscribers.length === 0 ? (
          <p className="text-text-secondary italic">Aucun abonné pour le moment.</p>
        ) : (
          <ul className="space-y-2">
            {subscribers.map((sub: any) => (
              <li key={sub.id} className="flex justify-between p-2 rounded bg-gray-50">
                <span className="text-text">{sub.user.firstName} {sub.user.lastName}</span>
                <span className="text-text-secondary text-sm">Expire le {new Date(sub.expiresAt).toLocaleDateString()}</span>
              </li>
            ))}
          </ul>
        )}
